...
def box3d_multiclass_nms(mlvl_bboxes,
    ...
    for i in range(0, num_classes):
        ...
        idx = torch.Tensor(range(_bboxes_for_nms.shape[0])).to(selected.device)
        not_selected = [i for i in idx if i not in selected]
        not_selected = selected.new_tensor(not_selected) # torch.Tensor(not_selected).to(selected.device)
        # not_selected = not_selected.int64()
        # print(selected.dtype, selected)
        # print(not_selected.dtype, not_selected)
        # assert len(selected) == _bboxes_for_nms.shape[0]
        _mlvl_bboxes = mlvl_bboxes[cls_inds, :]
        bboxes.append(_mlvl_bboxes[selected])
        bboxes.append(_mlvl_bboxes[not_selected])
        scores.append(_scores[selected])
        _scores[not_selected] /= 12 # _scores.min()
        scores.append(_scores[not_selected])
        cls_label = mlvl_bboxes.new_full(((_bboxes_for_nms.shape[0]), ),
                                         i,
                                         dtype=torch.long)

...
@PIPELINES.register_module()
class RandomFlip3DFS(RandomFlip):
    """Flip the points & bbox.
    ...
    def __init__(self,
                 ...
                 flip_ratio_bev_vertical=0.0,
                 **kwargs):
        super(RandomFlip3DFS, self).__init__(
            flip_ratio=flip_ratio_bev_horizontal, **kwargs)
        self.sync_2d = sync_2d
        self.flip_ratio_bev_vertical = flip_ratio_bev_vertical
        if flip_ratio_bev_horizontal is not None:
    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal'):
        ...
        if len(input_dict['bbox3d_fields']) == 0:  # test mode
            input_dict['bbox3d_fields'].append('empty_box3d')
            input_dict['bbox3d_fields'].append('empty_sbox3d')
            input_dict['bbox3d_fields'].append('empty_fbox3d')
            input_dict['empty_box3d'] = input_dict['box_type_3d'](
                np.array([], dtype=np.float32))
            input_dict['empty_sbox3d'] = input_dict['box_type_3d'](
                np.array([], dtype=np.float32))
            input_dict['empty_fbox3d'] = input_dict['box_type_3d'](
                np.array([], dtype=np.float32))
        assert len(input_dict['bbox3d_fields']) == 3
        for key in input_dict['bbox3d_fields']:
            if 'points' in input_dict:
                input_dict['points'] = input_dict[key].flip(
                    direction, points=input_dict['points'])
                ...
                input_dict[key].flip(direction)
        if 'centers2d' in input_dict and 'scenters2d' in input_dict and 'fcenters2d' in input_dict:
            assert self.sync_2d is True and direction == 'horizontal', \
                'Only support sync_2d=True and horizontal flip with images'
            w = input_dict['ori_shape'][1]
            input_dict['centers2d'][..., 0] = \
                w - input_dict['centers2d'][..., 0]
            input_dict['scenters2d'][..., 0] = \
                w - input_dict['scenters2d'][..., 0]
            input_dict['fcenters2d'][..., 0] = \
                w - input_dict['fcenters2d'][..., 0]
            # need to modify the horizontal position of camera center
            # along u-axis in the image (flip like centers2d)
            # ['cam2img'][0][2] = c_u
            # see more details and examples at
    ...
    def __call__(self, input_dict):
        ...
        """
        # flip 2D image and its annotations
        super(RandomFlip3DFS, self).__call__(input_dict)
